...
def like(request, uid):
    ...
    if post:
        ...
        LikeDislike.objects.create(
            user=request.user, film=film, dislike=False, like=True)
...
def dislike(request, uid):
    ...
    if post:
        ...
        LikeDislike.objects.create(
            user=request.user, film=film, like=False, dislike=True)
    ...
    def post(self, request, uid):
        ...
        if form.is_valid():
            if not Critique.objects.filter(user=request.user, film=film).exists():
                ...
                messages.error(
                    request, 'Impossible d\'ajouter une nouvelle critique pour ce film')
                return redirect('movie:film_detail', film.uid)
        return redirect('movie:film_detail', film.uid)
...
class DetailCritiqueView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['commentaires'] = context['critique'].commentaires.select_related(
            'user').all()
        print(context)
