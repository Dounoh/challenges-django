...
class CreateUserForm(UserCreationForm):
    ...
    password1 = forms.CharField(max_length=30,required=True,label='mot de pass',widget=forms.PasswordInput())
    password2 = forms.CharField(max_length=30,required=True,label='Confirmation',widget=forms.PasswordInput())
    profil = forms.ImageField(required=False)
    
    class Meta:
        model = User
        fields = ('first_name','last_name','email','password1','password2','role','profil')
        widgets = {
    ...
    def clean_password2(self):
        ...
        if not password1 or not password2:
            raise ValidationError('Champ obligatoire le champ mot de pass doit etre rempli')
...
class NewPasswordForm(forms.Form):
    password1 = forms.CharField(label='Nouveau mot de pass :',required=True,widget=forms.PasswordInput(attrs={'class':'form-control'}))
    password2 = forms.CharField(label='Confirmation :',required=True,widget=forms.PasswordInput(attrs={'class':'form-control'}))
    ...
    def clean_password(self):
        ...
        if not password1 or not password2:
            raise ValidationError('le champ mot de pass ne doit pas etre vide.')
        if password1 != password2:
            raise ValidationError('les deux mots de pass sont differents')
        
        if len(password2) < 5:
            raise ValidationError('le mot de pass doit etre supperieur à 5 elements')
        ...
        if not any(up.isupper() for up in password2):
            raise ValidationError('le mot de pass doit contenir au moin une lettre majuscule')
        if not any(low.islower() for low in password2):
            raise ValidationError('le mot de pass doit contenir au moin une lettre miniscule')
